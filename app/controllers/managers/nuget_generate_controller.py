...
async def nuget_create_package(
    ...
    all_versions = await get_all_versions("nuget", package_name=package_name)
    if all_versions:
...
async def nuget_search_new_versions(package: dict[str, Any]) -> None:
    ...
    all_versions = await get_all_versions("nuget", package_name=package["name"])
    counter = await count_number_of_versions_by_package(package["name"], "nuget")
    if counter < len(all_versions):
        cpe_matches = await read_cpe_product_by_package_name(package["name"])
        actual_versions = await read_versions_names_by_package(package["name"], "nuget")
        for version in all_versions:
            if version["release"] not in actual_versions:
                version["count"] = counter
                new_version = await attribute_cves(
                    version, cpe_matches, "nuget", package["name"]
                )
                no_existing_versions.append(new_version)
                counter += 1
    await update_package_moment(package["name"], "nuget")
