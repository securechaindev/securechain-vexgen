...
class JavaScriptTypeScriptAnalyzer(BaseCodeAnalyzer):
    def __init__(self):
        super().__init__()
        self.import_alias_pattern = compile(
            r"(?:import|const)\s+(\w+)\s*(?:=\s*require\(|from\s+)['\"]([\w\-/@\.]+)['\"]"
        )

    def get_import_pattern(self, import_name: str) -> str:
        return rf"(import\s+.*\s+from\s+['\"]{import_name}['\"]|require\(['\"]{import_name}['\"]\))"
    ...
    async def extract_patterns(
        ...
        for line in code.splitlines():
            match = self.import_alias_pattern.search(line)
            if match:
                alias = match.group(1)
        ...
        for target in import_names:
            ...
            patterns.extend([
                ...
                (rf"import\s+{{[^}}]+}}\s+from\s+['\"]{escaped}['\"]", "named_import", target),
                (rf"const\s+{{[^}}]+}}\s*=\s*require\(['\"]{escaped}['\"]\)", "named_import", target)
            ])
