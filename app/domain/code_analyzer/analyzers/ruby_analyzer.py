...
class RubyAnalyzer(BaseCodeAnalyzer):
    def __init__(self):
        super().__init__()
        self.alias_pattern = compile(r"(\w+)\s*=\s*([\w:]+)\.new")

    def get_import_pattern(self, import_name: str) -> str:
        return rf"(require|require_relative|include|extend)\s+['\"]?({escape(import_name)})['\"]?\b"
    ...
    def should_skip_line(self, line: str, inside_comment: bool) -> tuple[bool, bool]:
        ...
        if search(r"require|require_relative|include|extend", line):
            return True, inside_comment
    ...
    async def extract_patterns(
        ...
        for target in import_names:
            ...
            patterns.extend([
                ...
                (rf"require\s+['\"]({escaped}/[^'\"]+)['\"]", "split_by_slash", target),
                (rf"require_relative\s+['\"]({escaped}/[^'\"]+)['\"]", "split_by_slash", target),
                (rf"include\s+{escaped}::[^\s\(\)\.:;]+", "split_by_double_colon_include", target),
                (rf"extend\s+{escaped}::[^\s\(\)\.:;]+", "split_by_double_colon_extend", target)
            ])

        patterns.append((r"(\w+)\.(\w+)", "alias_method_call", None))
