...
async def process_sboms(GenerateVEXRequest: GenerateVEXRequest) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[str], str]:
    last_commit_date = await get_last_commit_date_github(
        GenerateVEXRequest.owner, GenerateVEXRequest.name
    )
    directory = await download_repository(GenerateVEXRequest.owner, GenerateVEXRequest.name)
    ...
    for sbom_file in sbom_files:
        ...
        if (
            ...
            sboms_names.append(relative_path)
    if sboms_to_process:
        result = await init_vex_tix(GenerateVEXRequest.owner, sboms_to_process, directory)
        for (vex, tix), sbom_name in zip(result, sboms_names):
            ...
            await update_user_tixs(tix_id, GenerateVEXRequest.user_id)
            vexs.append(vex)
            tixs.append(tix)
