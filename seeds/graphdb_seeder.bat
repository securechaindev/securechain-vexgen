...
docker exec --interactive --tty cargo_neo4j gzip -d /backups/cargo_neo4j.cypher.gz  > nul 2>&1
docker exec --interactive --tty cargo_neo4j cypher-shell -u %GRAPH_DB_USER% -p %GRAPH_DB_PASSWORD_CARGO% -a bolt://localhost:7685 -f /backups/cargo_neo4j.cypher > nul 2>&1
docker exec --interactive --tty cargo_neo4j rm /backups/cargo_neo4j.cypher
echo Cargo graph database was restored
docker exec --interactive --tty nuget_neo4j gzip -d /backups/nuget_neo4j.cypher.gz  > nul 2>&1
docker exec --interactive --tty nuget_neo4j cypher-shell -u %GRAPH_DB_USER% -p %GRAPH_DB_PASSWORD_NUGET% -a bolt://localhost:7685 -f /backups/nuget_neo4j.cypher > nul 2>&1
