...
docker exec --interactive --tty pypi_neo4j cypher-shell -u %GRAPH_DB_USER% -p %GRAPH_DB_PASSWORD_PIP% -a bolt://localhost:7687 -f /backups/pypi_neo4j.cypher > nul 2>&1
...
docker exec --interactive --tty maven_neo4j cypher-shell -u %GRAPH_DB_USER% -p %GRAPH_DB_PASSWORD_MVN% -a bolt://localhost:7685 -f /backups/maven_neo4j.cypher > nul 2>&1
